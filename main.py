...
def fetch_youtube_cookies_and_headers( 
    ...
    with sync_playwright() as p:
        ...
        # Extract user agent and language
        ...
        for c in cookies:
            ...
            expires = str(int(c["expires"])) if c.get("expires") else "0"
            name = c["name"]
            value = c["value"]
            line = "\t".join([domain, include_subdomains, path, secure, expires, name, value])
            lines.append(line)
...
def handleDownload(url: RequestedUrl):
    try:
        ...
        auth_data = fetch_youtube_cookies_and_headers()
